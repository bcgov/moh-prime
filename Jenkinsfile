        ...
        stage('Deploy Branch') {
            ...
            steps {
                script {
                    ...
                    echo "Deploy to dev..."
                    sh "printenv"
                    sh "./player.sh deploy postgres dev -p SUFFIX=${SUFFIX} -p VOLUME_CAPACITY=256Mi -p STORAGE_CLASS=netapp-file-standard -p STORAGE_MODE=ReadWriteMany"
                    sh "./player.sh deploy mongo dev -p SUFFIX=${SUFFIX} -p VOLUME_CAPACITY=256Mi -p STORAGE_CLASS=netapp-file-standard -p STORAGE_MODE=ReadWriteMany"
