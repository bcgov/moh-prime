...
namespace Prime.HttpClients
{
    public class VerifiableCredentialClient : IVerifiableCredentialClient
    {
        ...
        public async Task<string> GetSchemaId(string did)
        {
            ...
            try
            {
                response = await _client.GetAsync($"schemas/created?schema_version={PrimeEnvironment.Current.VerifiableCredentialApi.SchemaVersion}&schema_issuer_did={did}&schema_name={PrimeEnvironment.Current.VerifiableCredentialApi.SchemaName}");
            }
            catch (Exception ex)
            {
                await LogError(response, ex);
