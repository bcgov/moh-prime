        ...
        public async Task<(List<PharmanetTransactionLog> Logs, bool ExistsMoreLogs)> RetrieveLatestPharmanetTxLogsAsync(long lastKnownTxId)
        {
            ...
            var httpResponse = await _client.GetAsync(PrimeEnvironment.PrimeOdrApi.Url.SetQueryParams(
                new
                {
                    ...
                    // "id of last transaction log record received by client, response will return only transaction logs with an id greater than lastTxnId"
                    lastTxnId = lastKnownTxId,
                    // "optional parameter to return only nnn transaction log records, otherwise upto MAX_FETCH_SIZE remaining transaction logs are returned"
