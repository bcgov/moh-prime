...
namespace Prime.Configuration.Database
{
    public class SelfDeclarationVersionConfiguration : SeededTable<SelfDeclarationVersion>
    {
        ...
        public override IEnumerable<SelfDeclarationVersion> SeedData
        {
            ...
            {
                return new[] {
                    new SelfDeclarationVersion { Id = 1,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.Conviction, Text = "Are you, or have you ever been, subject to any limits, conditions or prohibitions imposed as a result of disciplinary actions taken by a governing body of a health profession in any jurisdiction, that involved improper access to, collection, use, or disclosure or retention of personal information?", EffectiveDate = InitialEffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                    new SelfDeclarationVersion { Id = 2,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.RegistrationSuspended, Text = "Are you, or have you ever been, the subject of an order or a conviction under legislation in any jurisdiction for a matter that involved improper access to, collection, use, or disclosure or retention of personal information?", EffectiveDate = InitialEffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                    ...
                    new SelfDeclarationVersion { Id = 5,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.Conviction, Text = "Have you ever been the subject of an order or conviction in British Columbia or any other jurisdiction for a matter involving an “unlawful or improper action”?", EffectiveDate = Dec2022EffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                    new SelfDeclarationVersion { Id = 6,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.RegistrationSuspended, Text = "Are you, or have you ever been, subject to the imposition, whether by order or with consent, of prohibitions, limits or conditions on your practice of a health profession: <ol style='list-style-type: lower-alpha;' class='mb-0'><li>in British Columbia, under the Health Professions Act or the Pharmacy Operations and Drug Scheduling Act, or</li><li>in any other jurisdiction, by a body that regulates a health profession in that jurisdiction</li></ol>for a matter involving an “unlawful or improper action”?", EffectiveDate = Dec2022EffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                    new SelfDeclarationVersion { Id = 7,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.DisciplinaryAction, Text = "Has your access to PharmaNet or any other health information system, whether or not electronic and whether or not in British Columbia or another jurisdiction, been suspended or cancelled for a matter involving an “unlawful or improper action”?", EffectiveDate = Dec2022EffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                    new SelfDeclarationVersion { Id = 8,  SelfDeclarationTypeCode = (int)SelfDeclarationTypeCode.PharmaNetSuspended, Text = "Has an employer ever disciplined you, or terminated your employment, for a matter involving an “unlawful or improper action”?  Has a contract for your services ever been terminated for a matter involving an “unlawful or improper action”?", EffectiveDate = Dec2022EffectiveDate, CreatedTimeStamp = SEEDING_DATE, UpdatedTimeStamp = SEEDING_DATE },
                };
            }
        }
    }
