...
namespace Prime.Services.Clients
{
    public class CollegeLicenceClient : ICollegeLicenceClient
    {
        ...
        public async Task<PharmanetCollegeRecord> GetCollegeRecordAsync(Certification certification)
        {
            ...
            try
            {
                response = await _client.PostAsync(PrimeEnvironment.PHARMANET_API_URL, requestParams.ToRequestContent());
            }
            catch (Exception ex)
            {
    ...
    public class CollegeLicenceClientHandler : HttpClientHandler
    {
        ...
        {
            ClientCertificates.Add(new X509Certificate2(PrimeEnvironment.PHARMANET_SSL_CERT_FILENAME, PrimeEnvironment.PHARMANET_SSL_CERT_PASSWORD));
