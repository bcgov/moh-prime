...
RUN dnf install -yqq https://download.postgresql.org/pub/repos/yum/reporpms/EL-8-x86_64/pgdg-redhat-repo-latest.noarch.rpm
RUN dnf search postgresql
RUN dnf provides psql
RUN dnf install -yqq postgresql10
RUN dnf install -yqq http://mirror.centos.org/centos/8-stream/AppStream/x86_64/os/Packages/xorg-x11-fonts-75dpi-7.5-19.el8.noarch.rpm && \
    dnf install -yqq https://github.com/wkhtmltopdf/packaging/releases/download/0.12.6-1/wkhtmltox-0.12.6-1.centos8.x86_64.rpm
    
# RUN dnf install -yqq libfontconfig1 libxrender1 libgdiplus xvfb
