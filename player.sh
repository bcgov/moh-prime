...
function deploy() {
    ...
    echo "$PROJECT_PREFIX\-$OC_APP"
    oc process -f "$TEMPLATE_DIRECTORY/$DEPLOY_CONFIG_TEMPLATE -p NAME=$OC_APP -p VERSION=$BUILD_NUMBER -p SUFFIX=$SUFFIX -p SOURCE_CONTEXT_DIR=$SOURCE_CONTEXT_DIR -p SOURCE_REPOSITORY_URL=$GIT_URL -p SOURCE_REPOSITORY_REF=$CHANGE_BRANCH -p OC_NAMESPACE=$PROJECT_PREFIX -p OC_APP=$OC_APP" | oc apply -f - --namespace="$PROJECT_PREFIX-$OC_APP"     
}
